...
// Revert any previous patch variants to keep the script idempotent.
// The right-hand anchor `, request?:` delimits the query option in current orval
// output. Older orval emitted `;` after the type, so we also handle that.
// Variant A: Omit with 'queryKey' | 'queryFn' (old bad patch)
content = content.replace(
  /query\?:\s*Omit<UseQueryOptions<([\s\S]*?)>, 'queryKey' \| 'queryFn'>(?=,\s*request\?:|;)/g,
  "query?: UseQueryOptions<$1>"
);
// Variant B: correct patch (Omit queryKey only + & { queryKey?: QueryKey })
content = content.replace(
  /query\?:\s*Omit<UseQueryOptions<([\s\S]*?)>, 'queryKey'> & \{ queryKey\?: QueryKey \}(?=,\s*request\?:|;)/g,
  "query?: UseQueryOptions<$1>"
);

const occurrences = (content.match(/query\?:\s*UseQueryOptions</g) || []).length;
...
// Replace `query?: UseQueryOptions<...>` followed by either `, request?:` (newer
// orval output) or `;` (older format) with the Omit/intersection form.
//
// Rationale:
...
content = content.replace(
  /query\?:\s*UseQueryOptions<([\s\S]*?)>(?=,\s*request\?:|;)/g,
  "query?: Omit<UseQueryOptions<$1>, 'queryKey'> & { queryKey?: QueryKey }"
);

const patched = (content.match(/query\?:\s*Omit<UseQueryOptions</g) || []).length;
