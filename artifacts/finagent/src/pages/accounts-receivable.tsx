...
import {
  ...
  getAccountReceivableRecurrenceInfo,
  useListCategories,
...
function ReceivableForm({ initial, profileId, onSave, onCancel }: {
  initial?: AccountReceivable | null;
  profileId: number;
    ...
    <div className="space-y-4 max-h-[75vh] overflow-y-auto pr-1">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <Input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Ex: Salário" />
        </div>
        <div>
          <Label>Valor (R$)</Label>
        ...
        <div>
          ...
          <Select value={form.categoryId != null ? String(form.categoryId) : "__none__"} onValueChange={v => setForm(f => ({ ...f, categoryId: v === "__none__" ? null : Number(v) }))}>
            <SelectTrigger><SelectValue placeholder="Selecionar..." /></SelectTrigger>
            <SelectContent>
              <SelectItem value="__none__">Nenhuma</SelectItem>
              {categories?.filter(c => c.isActive !== false && (c.type === "income" || c.type === "both")).map(c => <SelectItem key={c.id} value={String(c.id)}>{c.name}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        <div>
...
function ReceiveModal({ item, profileId, onClose }: { item: AccountReceivable; profileId: number; onClose: () => void }) {
  const receiveMutation = useReceiveAccountReceivable();
  const qc = useQueryClient();
  const { toast } = useToast();
  const today = new Date().toISOString().split("T")[0];
  const [receivedAt, setReceivedAt] = useState(today);
  const [receivedAmount, setReceivedAmount] = useState(item.amount);
  ...
  const handleReceive = async () => {
    if (!bankAccountId) {
      toast({ title: "Selecione uma conta bancária para dar baixa.", variant: "destructive" });
      return;
    }
    try {
      await receiveMutation.mutateAsync({ id: item.id, data: { receivedAt, receivedAmount, bankAccountId } });
      toast({ title: "Recebimento registrado!" });
      qc.invalidateQueries();
      onClose();
    } catch (err: unknown) {
      const msg = (err as { message?: string })?.message ?? "Erro ao registrar recebimento";
    ...
    <DialogContent>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <CurrencyInput value={receivedAmount} onChange={setReceivedAmount} />
        </div>
        <div>
          <Label>Conta Bancária (crédito) <span className="text-destructive">*</span></Label>
          <Select value={String(bankAccountId ?? "")} onValueChange={v => setBankAccountId(v ? Number(v) : null)}>
            <SelectTrigger className={!bankAccountId ? "border-destructive" : ""}><SelectValue placeholder="Selecionar conta..." /></SelectTrigger>
            <SelectContent>
              {bankAccounts?.filter(b => b.isActive).map(b => <SelectItem key={b.id} value={String(b.id)}>{b.name}</SelectItem>)}
            </SelectContent>
          </Select>
          {!bankAccountId && <p className="text-xs text-destructive mt-1">Obrigatório para registrar recebimento</p>}
        </div>
        <div className="flex gap-2 justify-end">
          <Button variant="outline" onClick={onClose}>Cancelar</Button>
          <Button onClick={handleReceive} disabled={!bankAccountId}>Confirmar Recebimento</Button>
        </div>
      </div>
    </DialogContent>
  );
...
export default function AccountsReceivablePage() {
  ...
  const { data: items, isLoading } = useListAccountsReceivable(
    {
      profileId: activeProfileId!,
      ...(filterYear ? { year: filterYear } : {}),
      ...(filterMonth ? { month: filterMonth } : {}),
      ...(filterStatus ? { status: filterStatus as AccountReceivableStatus } : {}),
    },
    { query: { enabled: !!activeProfileId } }
  );

  const createItem = useCreateAccountReceivable();
  const updateItem = useUpdateAccountReceivable();
  const deleteItem = useDeleteAccountReceivable();

  const [showForm, setShowForm] = useState(false);
  const [editItem, setEditItem] = useState<AccountReceivable | null>(null);
  const [receiveItem, setReceiveItem] = useState<AccountReceivable | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<{ item: AccountReceivable; recurrenceGroupId: string | null; futureCount: number } | null>(null);

  const handleSave = async (data: ReceivableFormData) => {
    try {
  ...
  const handleDeleteClick = async (item: AccountReceivable) => {
    try {
      const info = await getAccountReceivableRecurrenceInfo(item.id);
  ...
  const filteredItems = filterSearch
    ? items?.filter(i => i.description.toLowerCase().includes(filterSearch.toLowerCase()))
    : items;
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      received: sum(i => i.status === "received"),
      receivedCount: count(i => i.status === "received"),
    };
  }, [filteredItems]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="border-green-500/30 bg-green-500/5">
          <CardContent className="p-4">
            <p className="text-xs text-green-600 dark:text-green-400 uppercase tracking-wide mb-1">Recebido</p>
            <p className="text-lg font-bold tabular-nums text-green-600 dark:text-green-400">{formatCurrency(totals.received)}</p>
            <p className="text-xs text-muted-foreground mt-0.5">{totals.receivedCount} lançamento(s)</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={showForm} onOpenChange={open => { if (!open) { setShowForm(false); setEditItem(null); } }}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editItem ? "Editar Conta" : "Nova Conta a Receber"}</DialogTitle></DialogHeader>
          <ReceivableForm
            initial={editItem}
            profileId={activeProfileId}
            onSave={handleSave}
      ...
      {receiveItem && (
        <Dialog open={!!receiveItem} onOpenChange={open => { if (!open) setReceiveItem(null); }}>
          <ReceiveModal item={receiveItem} profileId={activeProfileId} onClose={() => setReceiveItem(null)} />
        </Dialog>
      )}
            ...
            <AlertDialogDescription>
              {deleteTarget && deleteTarget.recurrenceGroupId !== null ? (
                <>
                  ...
                  {" "}Parcelas já recebidas e parcelas com vencimento anterior a hoje não serão afetadas.
                </>
              ) : (
